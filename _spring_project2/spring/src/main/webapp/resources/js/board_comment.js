		...
		if (result.length > 0) { 
			...
			for (let cvo of result) {
				let li = `
					<li data-cno="${cvo.cno}">
						<div>
							<p>${cvo.writer} (${cvo.regdate})</p>
							<div style="display: flex;align-items: flex-end;"	>
								<input type="text" value="${cvo.content}" class="cmtTextMod input-text" style="flex: auto;"> 	
								<button type="button" data-cno="${cvo.cno}" class="cmtModBtn button-green">수정</button>
								<button type="button" data-cno="${cvo.cno}" class="cmtDelBtn button-red">삭제</button>
							</div>
						</div>
					</li>`;
				ul.innerHTML += li;
